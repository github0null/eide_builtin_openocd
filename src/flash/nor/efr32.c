...
static int get_efr32x_info(struct flash_bank *bank, struct command_invocation *cmd)
{
	...
	ret = efr32x_decode_info(&info, buf, sizeof(buf));
	if (ERROR_OK != ret) {
		LOG_ERROR("Failed on call 'efr32x_decode_info'");
		return ret;
	}
